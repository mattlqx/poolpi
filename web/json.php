...
$data['probe_temp_f'] = sprintf("%0.02f", $probe_temp);
$data['outdoor_temp_f'] = sprintf("%0.02f", $outdoor_temp);
...
foreach ($last_day as $line) {
  if (preg_match("/(\d+): (\d+\.\d+e\+01) (\d+\.\d+e\+01)/", $line, $matches)) {
    if ($matches[2] > $data['peak_temp_f']) {
      $data['peak_temp_f'] = sprintf("%0.02f", $matches[2]);
      $data['peak_temp_time'] = $matches[1];
    }
  }
}
